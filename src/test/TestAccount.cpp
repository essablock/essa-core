...
TestAccount::updateSequenceNumber()
{
    if (mSn == 0)
    {
        ...
        auto entry = essa::loadAccount(ltx, getPublicKey());
        if (entry)
        {
...
TestAccount::create(SecretKey const& secretKey, uint64_t initialBalance)
{
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto entry = essa::loadAccount(ltx, publicKey);
        if (entry)
        {
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto destAfter = essa::loadAccount(ltx, publicKey);
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        REQUIRE(essa::loadAccount(ltx, publicKey));
    }
...
TestAccount::merge(PublicKey const& into)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    REQUIRE(essa::loadAccount(ltx, into));
    REQUIRE(!essa::loadAccount(ltx, getPublicKey()));
}
...
TestAccount::manageData(std::string const& name, DataValue* value)
{
    ...
    auto data = essa::loadData(ls, getPublicKey(), name);
    if (value)
    {
        REQUIRE(data);
...
TestAccount::pay(PublicKey const& destination, int64_t amount)
{
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto toAccountEntry = essa::loadAccount(ltx, destination);
        toAccount =
            toAccountEntry
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto toAccountAfter = essa::loadAccount(ltx, destination);
        // check that the target account didn't change
