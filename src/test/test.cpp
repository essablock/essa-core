...
bool force_sqlite = (std::getenv("ESSABLOCKCHAIN_FORCE_SQLITE") != nullptr);
...
getTestConfig(int instanceNumber, Config::TestDbMode mode)
{
    ...
    {
        gTestRoots.emplace_back("essa-core-test");
        ...
        sstream << "essa" << instanceNumber << ".log";
     ...
     std::vector<std::string> const& metrics)
{
    ...
    // not mixed with essa-core logging.
    Logging::setFmt("<test>");
    ...
    LOG(INFO) << "Testing essa-core " << ESSABLOCKCHAIN_CORE_VERSION;
    ...
    cli |= clara::Opt(gBaseInstance, "offset")["--base-instance"](
        ...
        "essa-core can run tests concurrently");
    session.cli(cli);
