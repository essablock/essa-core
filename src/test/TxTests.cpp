              ...
              std::vector<TransactionFramePtr> const& txs)
{
    ...
    EssaBlockChainValue sv(txSet->getContentsHash(), getTestDate(day, month, year),
                    emptyUpgradeSteps, 0);
...
{
    auto res = essa::loadAccountWithoutRecord(ltx, k);
    if (mustExist)
    {
        REQUIRE(res);
    }
...
{
    LedgerTxn ltx(app.getLedgerTxnRoot());
    return (bool)essa::loadAccountWithoutRecord(ltx, k);
}
...
{
    LedgerTxn ltx(app.getLedgerTxnRoot());
    auto account = essa::loadAccount(ltx, k);
                       ...
                       SequenceNumber seq)
{
    ...
    case MANAGE_OFFER_UPDATED:
    {
        LedgerTxn ltx(app.getLedgerTxnRoot());
        auto offer =
            essa::loadOffer(ltx, source.getPublicKey(), expectedOfferID);
        REQUIRE(offer);
                        ...
                        ManageOfferEffect expectedEffect)
{
    ...
    if (createPassiveOfferResult.code() == MANAGE_OFFER_SUCCESS)
    {
        ...
        {
            LedgerTxn ltx(app.getLedgerTxnRoot());
            REQUIRE(!essa::loadOffer(ltx, source.getPublicKey(),
                                        expectedOfferID));
        }
        break;
        default:
