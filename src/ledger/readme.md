# What is a ledger?
A ledger represents the state of the EssaBlockChain universe at a given point in time.
...
[`src/xdr/EssaBlockChain-ledger.x`](../xdr/EssaBlockChain-ledger.x)
...
During consensus, nodes work together to decide on the value of EssaBlockChainValue.
EssaBlockChainValue is then saved in the scpValue field of the ledger header.
Any node on the network, given the previous ledger (their previous state) and
'EssaBlockChainValue' should be able to transition to the same new ledger.
...
[`src/xdr/EssaBlockChain-ledger-entries.x`](../xdr/EssaBlockChain-ledger-entries.x)

### AccountEntry
This entry represents an account. In EssaBlockChain, everything is centered around
...
Offers are a way to automate simple trading inside the EssaBlockChain network.
