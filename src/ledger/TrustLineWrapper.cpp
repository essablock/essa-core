...
TrustLineWrapper::NonIssuerImpl::addBalance(LedgerTxnHeader const& header,
                                            int64_t delta)
{
    return essa::addBalance(header, mEntry, delta);
}
...
{
    return essa::getBuyingLiabilities(header, mEntry);
}
...
TrustLineWrapper::NonIssuerImpl::getSellingLiabilities(
    LedgerTxnHeader const& header)
{
    return essa::getSellingLiabilities(header, mEntry);
}
...
{
    return essa::addBuyingLiabilities(header, mEntry, delta);
}
...
TrustLineWrapper::NonIssuerImpl::addSellingLiabilities(
    LedgerTxnHeader const& header, int64_t delta)
{
    return essa::addSellingLiabilities(header, mEntry, delta);
}

bool
TrustLineWrapper::NonIssuerImpl::isAuthorized() const
{
    return essa::isAuthorized(mEntry);
}
...
{
    return essa::getAvailableBalance(header, mEntry);
}
...
TrustLineWrapper::NonIssuerImpl::getMaxAmountReceive(
    LedgerTxnHeader const& header) const
{
    return essa::getMaxAmountReceive(header, mEntry);
}
