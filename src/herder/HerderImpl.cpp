...
void
HerderImpl::valueExternalized(uint64 slotIndex, EssaBlockChainValue const& value)
{
    // record metrics
...
HerderImpl::recvTransaction(TransactionFramePtr tx)
{
...

    {
        ...
        auto sourceAccount = essa::loadAccount(ltx, tx->getSourceID());
...
HerderImpl::triggerNextLedger(uint32_t ledgerSeqToTrigger)
{
    ...
    EssaBlockChainValue newProposedValue(txSetHash, nextCloseTime, emptyUpgradeSteps,
                                  0);
    ...
    std::vector<TransactionFramePtr> const& applied)
{
    ...
    // propagation
    {
        ...
        {
            auto msg = tx->toEssaBlockChainMessage();
