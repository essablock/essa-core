...
void
save(Archive& ar, essa::Upgrades::UpgradeParameters const& p)
{
    ar(make_nvp("time", essa::VirtualClock::to_time_t(p.mUpgradeTime)));
...
template <class Archive>
void
load(Archive& ar, essa::Upgrades::UpgradeParameters& o)
{
    time_t t;
    ar(make_nvp("time", t));
    o.mUpgradeTime = essa::VirtualClock::from_time_t(t);
               ...
               AccountID const& accountID, Asset const& asset, int64_t delta)
{
    ...
    if (iter->second)
    {
        if (!essa::addBalance(*iter->second, delta))
        {
            iter->second.reset();
        }
    }
                                        ...
                                        AbstractLedgerTxn& ltx)
{
    ...
    {
        auto trust = essa::loadTrustLineWithoutRecord(ltx, accountID, asset);
...
prepareLiabilities(AbstractLedgerTxn& ltx, LedgerTxnHeader const& header)
{
    ...
    for (auto& accountOffers : offersByAccount)
    {
        ...
        auto accountEntry = essa::loadAccount(ltx, accountOffers.first);
        if (!accountEntry)
        {
        ...
        for (auto& offerEntry : accountOffers.second)
        {
            ...
            {
                essa::addNumEntries(header, accountEntry, -1);
            }
        ...
        for (auto const& assetLiabilities : liabilities)
        {
            ...
            {
                auto trustEntry =
                    essa::loadTrustLine(ltx, accountOffers.first, asset);
                int64_t deltaSelling =
