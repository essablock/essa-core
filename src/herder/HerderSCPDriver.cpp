...
#include "xdr/EssaBlockChain-SCP.h"
...
void
HerderSCPDriver::restoreSCPState(uint64_t index, EssaBlockChainValue const& value)
{
...
HerderSCPDriver::checkCloseTime(uint64_t slotIndex, uint64_t lastCloseTime,
                                EssaBlockChainValue const& b) const
{
                                   ...
                                   std::set<Value> const& candidates)
{
    ...
    EssaBlockChainValue comp(h, 0, emptyUpgradeSteps, 0);
    ...
    std::vector<EssaBlockChainValue> candidateValues;

    for (auto const& c : candidates)
    {
        candidateValues.emplace_back();
        EssaBlockChainValue& sv = candidateValues.back();
...
HerderSCPDriver::valueExternalized(uint64_t slotIndex, Value const& value)
{
    ...
    catch (...)
    {
        ...
        // therefore contain a valid EssaBlockChainValue.
        CLOG(ERROR, "Herder") << "HerderSCPDriver::valueExternalized"
                              << " Externalized EssaBlockChainValue malformed";
...
void
HerderSCPDriver::nominate(uint64_t slotIndex, EssaBlockChainValue const& value,
                          TxSetFramePtr proposedSet,
                          EssaBlockChainValue const& previousValue)
{
