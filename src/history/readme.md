...
The `get` and `put` commands define subprocesses that `essa-core` will
...
called `s3essa` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3essa]
get="curl http://history.essa.org/{0} -o {1}"
put="aws s3 cp {0} s3://history.essa.org/{1}"
~~~~

In this example, `essa-core` will use the `curl` command to fetch history and
