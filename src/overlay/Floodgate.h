...
class Floodgate
{
    class FloodRecord
    {
      public:
        ...
        uint32_t mLedgerSeq;
        EssaBlockChainMessage mMessage;
        std::set<std::string> mPeersTold;

        FloodRecord(EssaBlockChainMessage const& msg, uint32_t ledger,
                    Peer::pointer peer);
    };
  ...
  public:
    ...
    bool addRecord(EssaBlockChainMessage const& msg, Peer::pointer fromPeer);

    void broadcast(EssaBlockChainMessage const& msg, bool force);
