...
namespace essa
{
Floodgate::FloodRecord::FloodRecord(EssaBlockChainMessage const& msg, uint32_t ledger,
                                    Peer::pointer peer)
...
bool
Floodgate::addRecord(EssaBlockChainMessage const& msg, Peer::pointer peer)
{
    if (mShuttingDown)
    {
        return false;
...
void
Floodgate::broadcast(EssaBlockChainMessage const& msg, bool force)
{
    if (mShuttingDown)
    {
        return;
