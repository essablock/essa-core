...
const struct option essa_core_options[] = {
...
usage(int err = 1)
{
    ...
    os << "usage: essa-core [OPTIONS]\n"
          ...
          "      --forcescp           Next time essa-core is run, SCP will "
          "start "
          ...
          "signtxn\n"
          "                           (Default is ESSABLOCKCHAIN_NETWORK_ID "
...
handleDeprecatedCommandLine(int argc, char* const* argv)
{
    ...
    std::string cfgFile("essa-core.cfg");
    std::string command;
    ...
    int opt;
    while ((opt = getopt_long_only(argc, argv, "c:", essa_core_options,
                                   nullptr)) != -1)
    {
        switch (opt)
        {
...
#endif
        case OPT_VERSION:
            std::cout << ESSABLOCKCHAIN_CORE_VERSION << std::endl;
            return 0;
        case OPT_HELP:
        default:
            usage(0);
