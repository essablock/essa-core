           ...
           std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    {
        auto accountB = essa::loadAccountWithoutRecord(ltx, accountBID);
        if (!accountB)
        {
    ...
    { // entire offer is taken
        ...
        auto accountB = essa::loadAccount(ltx, accountBID);
    ...
    if (numSheepSend != 0)
    {
        ...
        if (sheep.type() == ASSET_TYPE_NATIVE)
        {
            auto accountB = essa::loadAccount(ltxInner, accountBID);
        ...
        {
            auto sheepLineAccountB =
                essa::loadTrustLine(ltxInner, accountBID, sheep);
    ...
    if (numWheatReceived != 0)
    {
        ...
        {
            auto wheatLineAccountB =
                essa::loadTrustLine(ltxInner, accountBID, wheat);
              ...
              bool isPathPayment, std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    if (!essa::loadAccountWithoutRecord(ltx, accountBID))
    {
        throw std::runtime_error(
            "invalid database state: offer must have matching account");
    }
    ...
    {
        accountB = essa::loadAccount(ltx, accountBID);
    }
                                   ...
                                   : CrossOfferResult::eOfferPartial;
    {
        ...
        {
            sellingWheatOffer.erase();
            accountB = essa::loadAccount(ltxInner, accountBID);
