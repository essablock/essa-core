                              ...
                              AccountID const& accountID)
{
    ...
    {
        // this is buggy caching that existed in old versions of the protocol
        auto res = essa::loadAccount(ltx, accountID);
        if (res)
        {
    ...
    {
        return essa::loadAccount(ltx, accountID);
    }
}
...
TransactionFrame::processFeeSeqNum(AbstractLedgerTxn& ltx)
{
    ...
    if (fee > 0)
    {
        ...
        essa::addBalance(acc.balance, -fee);
...
{
    auto account = essa::loadAccount(ltx, accountID);
    if (!account)
    {
                                      ...
                                      SignerKey const& signerKey) const
{
    ...
    {
        auto removed = essa::addNumEntries(header, account, -1);
        assert(removed);
...
EssaBlockChainMessage
TransactionFrame::toEssaBlockChainMessage() const
{
