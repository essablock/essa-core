...
PathPaymentOpFrame::doApply(Application& app, AbstractLedgerTxn& ltx)
{
    ...
    if (ltx.loadHeader().current().ledgerVersion < 8)
    {
        doesSourceAccountExist =
            (bool)essa::loadAccountWithoutRecord(ltx, getSourceID());
    }

    if (!bypassIssuerCheck)
    {
        if (!essa::loadAccountWithoutRecord(ltx, mPathPayment.destination))
        {
    ...
    {
        auto destination = essa::loadAccount(ltx, mPathPayment.destination);
    ...
    else
    {
        ...
        {
            auto issuer =
                essa::loadAccountWithoutRecord(ltx, getIssuer(curB));
            if (!issuer)
            {
        ...
        auto destLine =
            essa::loadTrustLine(ltx, mPathPayment.destination, curB);
        if (!destLine)
        {
    ...
    for (int i = (int)fullPath.size() - 1; i >= 0; i--)
    {
        ...
        {
            if (!essa::loadAccountWithoutRecord(ltx, getIssuer(curA)))
            {
                innerResult().code(PATH_PAYMENT_NO_ISSUER);
    ...
    if (curB.type() == ASSET_TYPE_NATIVE)
    {
        ...
        {
            sourceAccount = essa::loadAccount(ltx, getSourceID());
            if (!sourceAccount)
            {
                innerResult().code(PATH_PAYMENT_MALFORMED);
                return false;
